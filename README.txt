...
Installation:
	1.	Ensure Python2.7 is installed on the system, /usr/bin/python should start python2.7
	2.	Load files to your Splunks scripts directory /<splunk installation>/bin/scripts
	3.	Make s2r.py executable:
		chmod +x s2r.py
	4.	Update s2r.conf settings with an editor to match your needs
	5.	Test connectivity to your Resilient instance:
		./s2r.py --testconnection
	6.	Add script s2r.py as an option to any alert in Splunk you want tickets auto created after execution.


Command Options:
	./s2r.py --help
	./s2r.py -c --checkqueue
	./s2r.py -p --processqueue
	./s2r.py -t --testconnection
...
Normal usage:
	...
	More Examples:
		...
		Example:	oa_emailaddress a_src_ip	a_filehash o_somefield
...
Recovery:
	...
	You can check the queue by executing  /<splunk installation>/bin/scripts/s2r.py --checkqueue
	Will return number of stored events.

	You can attempt to send stored events by excuting /<splunk installation>/bin/scripts/s2r.py --processqueue
...
Other:
	...
	This is to help resolve possible dependancies issues.